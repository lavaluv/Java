...
/*
 * 数据库设计范式：
 * 1NF:在关系模型中,数据库表的每一列都是不可分割的原子数据项,而不能是集合，数组，记录等非原子数据项。
 * 2NF:要求数据库表中的每个实例或记录必须可以被唯一地区分。选取一个能区分每个实体的属性或属性组，作为实体的唯一标识,即实体的属性完全依赖于主关键字。
 ...
 * 事务并发的问题：脏读（事务A读取了事务B更新的数据，然后事务B在某些因素下执行了回滚，那么事务A读取的数据就是不合理的，即脏数据。）
 * 不可重复读（事务A需要重复多次读取某组数据，事务A在事务B对该组数据修改提交前后进行读取，很显然、两次读取的数据是不一致的，即不可重复读。侧重于元数据的修改。）
 * 幻读（事务A在修改每一条元数据的时候，事务B在此时添加了一条新记录，事务A在处理的过程中突然多了一条数据，即幻读。侧重于数据的删除与修改。）
 * 
 ...
 * read committed 锁住整行，serializable 锁住整张表；read committed解决脏读，repeatable read解决不可重复读，serailizable解决幻读
 ...
 * 4.使用不等于(!= 或者<>)的时候，无法使用索引，会导致索引失效
 * 5.like查询是以%开头，索引会失效变成全表扫描，覆盖索引。
 * 6.如果条件中有or，即使其中有条件带索引也不会使用(这也是为什么尽量少用or的原因)。要想使用or，又想让索引生效，只能将or条件中的每个列都加上索引
 * 7.如果mysql使用全表扫描要比使用索引快,则不会使用到索引
 * 8.如果列类型是字符串，要使用引号。例如where A='China'，否则索引失效（会进行类型转换）；
 * 何时使用索引：
 ...
 * 
 * mysql分页基本语法：SELECT * FROM table LIMIT [offset,] rows 
